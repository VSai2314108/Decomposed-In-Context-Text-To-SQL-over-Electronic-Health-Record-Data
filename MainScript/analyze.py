...
df = pandas.read_csv("/blue/cap4773/somasundaramv/Few-shot-NL2SQL-with-prompting/LLAMA-RESULTS-CSV.csv")
print(df)
...
#             print()
    k += 1
print('Execution Accuracy: {}'.format(cnt/100))
df = pandas.DataFrame(results, columns=['NLQ', 'PRED SQL', 'GOLD SQL', 'OUTPUT PRED', 'OUTPUT GOLD'])
df.to_csv("/blue/cap4773/somasundaramv/Few-shot-NL2SQL-with-prompting/TREQS/evaluation/analysis_of_results.csv", index=False)