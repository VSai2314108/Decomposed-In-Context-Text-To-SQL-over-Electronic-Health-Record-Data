...
import os
import sys
from llama import TextGenerator
...
def load_data(DATASET):
  try:
    ...
    return pd.read_json(DATASET, lines = True)      
...
def hard_prompt_maker(test_sample_text,database,schema_links,sub_questions):
  ...
  fields = find_fields_MYSQL_like("medical")
  fields += "Foreign_keys = " + find_foreign_keys_MYSQL_like("medical") + '\n'
...
def creatiing_schema(DATASET_JSON):
    ...
    for index, row in schema_df.iterrows():
        ...
        for col, col_type in zip(col_names, col_types):
            ...
            if index == -1:
                for table in tables:
                    schema.append(["medical", table, '*', 'text'])
            else:
                schema.append(["medical", tables[index], col_name, col_type])
        for primary_key in primary_keys:
            index, column = col_names[primary_key]
            p_keys.append(["medical", tables[index], column])
        for foreign_key in foreign_keys:
            first, second = foreign_key
            first_index, first_column = col_names[first]
            second_index, second_column = col_names[second]
            f_keys.append(["medical", tables[first_index], tables[second_index], first_column, second_column])
...
def GPT4_debug(prompt):
  response = text_generator.generate_text(prompt,350)
  return response
...
if __name__ == '__main__':
    ...
    for index, row in val_df.iterrows():
        ...
        while debugged_SQL is None:
            try:
                debugged_SQL = GPT4_debug(debuger(row['question_refine'], "medical", SQL)).replace("\n", " ")
            except:
                time.sleep(3)
                pass
        ...
        print("Gold:" + row['sql'])
        print("\n#############################################################################\n")
        CODEX.append([row['question_refine'], SQL, row['sql'], "medical"])
        df = pd.DataFrame(CODEX, columns=['NLQ', 'PREDICTED SQL', 'GOLD SQL', 'DATABASE'])
        df.to_csv("LLAMA-RESULTS-CSV.csv")
    df = pd.DataFrame(CODEX, columns=['NLQ', 'PREDICTED SQL', 'GOLD SQL', 'DATABASE'])
    df.to_csv("LLAMA-RESULTS-CSV.csv")
